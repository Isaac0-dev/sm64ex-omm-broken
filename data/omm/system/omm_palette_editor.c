...
#define OMM_PE_TEXT_X                   (GFX_DIMENSIONS_FROM_LEFT_EDGE(OMM_PE_DISPLAY_X))
...
#define OMM_PE_DROPDOWN_X               (OMM_PE_DROPDOWN_HOVER_X - (OMM_PE_DROPDOWN_MARGIN - OMM_PE_DROPDOWN_HOVER_MARGIN))
#define OMM_PE_DROPDOWN_Y(_y_)          (OMM_PE_DROPDOWN_HOVER_Y(_y_))
#define OMM_PE_DROPDOWN_W               (OMM_PE_DROPDOWN_HOVER_W + 2 * (OMM_PE_DROPDOWN_MARGIN - OMM_PE_DROPDOWN_HOVER_MARGIN))
...
static void omm_palette_editor_set_current() {
    pe->group = clamp_s(pe->group, 0, omm_mario_colors_light_groups_count(pe->peach));
...
static void omm_palette_editor_render_string(const u8 *str, s32 maxlen, s32 x, s32 y, s32 w, s32 h, u8 r, u8 g, u8 b, u8 a, bool shadow) {
...
static void omm_palette_editor_render_color_picker(f32 wx, f32 wy) {
    ...
    omm_render_rect_window_coords(wx + dn0 + 1, wy + dn0 + 1, w0 - 2, w1 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dn0 + 1, wy + dp0 + 1, w0 - 2, w1 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dn0 + 1, wy + dn0 + 1, w1 - 2, w0 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dp0 + 1, wy + dn0 + 1, w1 - 2, w0 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dn1 + 1, wy + dn2 + 1, w2 - 2, w1 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dp1 + 1, wy + dn2 + 1, w2 - 2, w1 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dn2 + 1, wy + dn1 + 1, w1 - 2, w2 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
    omm_render_rect_window_coords(wx + dn2 + 1, wy + dp1 + 1, w1 - 2, w2 - 2, 0xFF, 0xFF, 0xFF, 0xFF);
}
...
static void omm_geo_palette_editor_render_colormap(s32 k) {
    omm_render_texrect(pe->colormaps[k]->x, pe->colormaps[k]->y, pe->colormaps[k]->w, pe->colormaps[k]->h, 32, 32, 0xFF, 0xFF, 0xFF, 0xFF, OMM_TEXTURE_MISC_WHITE, false);
    if (pe->bright) {
        ...
        vtx[1] = (Vtx) {{{ x1, y0, 0 }, 0, { (0x400 * pe->lights[k]->u + 1) * 32.f, 0x2000 }, { 0xFF, 0xFF, 0xFF, 0xFF }}};
        vtx[2] = (Vtx) {{{ x0, y1, 0 }, 0, { (0x400 * pe->lights[k]->u + 0) * 32.f, 0x0000 }, { 0x00, 0x00, 0x00, 0xFF }}};
        vtx[3] = (Vtx) {{{ x1, y1, 0 }, 0, { (0x400 * pe->lights[k]->u + 1) * 32.f, 0x0000 }, { 0xFF, 0xFF, 0xFF, 0xFF }}};
        ...
        omm_render_texrect(pe->colormaps[k]->x + 1, pe->colormaps[k]->y + 1, pe->colormaps[k]->w - 2, pe->colormaps[k]->h - 2, 1024, 256, pe->lights[k]->w * 0xFF, pe->lights[k]->w * 0xFF, pe->lights[k]->w * 0xFF, 0xFF, OMM_TEXTURE_MISC_COLOR_MAP, false);
...
static Gfx *omm_geo_palette_editor_render(s32 callContext, UNUSED struct GraphNode *node, UNUSED void *context) {
    if (callContext == GEO_CONTEXT_RENDER && pe->state == OMM_PE_STATE_OPEN) {
        ...
        omm_palette_editor_render_string(textPlayer, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_PLAYER_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textPlayerName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_PLAYER_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
        ...
        omm_palette_editor_render_string(textPalette, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_PALETTE_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textPaletteName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_PALETTE_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
        ...
        omm_palette_editor_render_string(textGroup, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_GROUP_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textGroupName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_GROUP_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);

        // Lights
        if (pe->group == 0) {
            s32 palette = omm_mario_colors_count() - 4 + pe->palette;
            s32 numGroups = omm_mario_colors_light_groups_count(pe->peach);
            ...
            for (s32 group = 0; group != numGroups; ++group) {
                ...
                if (group == pe->subgroup) {
                    omm_palette_editor_render_string(textGroupName, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0x00, 0xFF, true);
                } else {
                    omm_palette_editor_render_string(textGroupName, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
                }
                ...
                for (s32 k = 0; k != 2; ++k) {
                    ...
                    omm_palette_editor_render_string(textColorValue, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2 + dx, y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF * (group != pe->subgroup), 0xFF, true);
                    omm_render_texrect(OMM_PE_COLOR_BOX_X + dx, y + OMM_PE_COLOR_BOX_Y, OMM_PE_COLOR_BOX_W, OMM_PE_COLOR_BOX_H, 32, 32, 0xFF, 0xFF, 0xFF, 0xFF, OMM_TEXTURE_MISC_WHITE, false);
                    omm_render_texrect(OMM_PE_COLOR_BOX_X + dx + 1, y + OMM_PE_COLOR_BOX_Y + 1, OMM_PE_COLOR_BOX_W - 2, OMM_PE_COLOR_BOX_H - 2, 32, 32, (color >> 16) & 0xFF, (color >> 8) & 0xFF, (color >> 0) & 0xFF, 0xFF, OMM_TEXTURE_MISC_WHITE, false);
                }
            }
            ...
            for (s32 k = 0; k != 2; ++k) {
                ...
                // Light name
                u8 *textLightName = omm_text_convert(omm_mario_colors_get_light_name(pe->peach, pe->subgroup * 2 + k), false);
                omm_palette_editor_render_string(textLightName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X, y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
            ...
            u8 *textColorValue = omm_text_convert(colorValueStr, false);
            omm_palette_editor_render_string(textColor, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_COLOR_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
            omm_palette_editor_render_string(textColorValue, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_COLOR_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
            omm_render_texrect(OMM_PE_COLOR_BOX_X, OMM_PE_TEXT_COLOR_Y + OMM_PE_COLOR_BOX_Y, OMM_PE_COLOR_BOX_W, OMM_PE_COLOR_BOX_H, 32, 32, 0xFF, 0xFF, 0xFF, 0xFF, OMM_TEXTURE_MISC_WHITE, false);
            omm_render_texrect(OMM_PE_COLOR_BOX_X + 1, OMM_PE_TEXT_COLOR_Y + OMM_PE_COLOR_BOX_Y + 1, OMM_PE_COLOR_BOX_W - 2, OMM_PE_COLOR_BOX_H - 2, 32, 32, pe->lights[k]->r * 0xFF, pe->lights[k]->g * 0xFF, pe->lights[k]->b * 0xFF, 0xFF, OMM_TEXTURE_MISC_WHITE, false);
        ...
        omm_palette_editor_render_string(textBackground, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_BACKGROUND_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textBackgroundName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_BACKGROUND_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
        ...
        omm_palette_editor_render_string(textCapState, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_CAP_STATE_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textCapStateName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_CAP_STATE_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
        ...
        omm_palette_editor_render_string(textAnimations, OMM_PE_STR_MAX_LENGTH_LEFT, OMM_PE_TEXT_X, OMM_PE_TEXT_ANIMATIONS_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        omm_palette_editor_render_string(textAnimationsName, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, OMM_PE_TEXT_ANIMATIONS_Y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, pe->anims ? 0x00 : 0xFF, pe->anims ? 0xFF : 0x00, 0x00, 0xFF, true);

        // Events
        if (pe->event->dropDown) {
            omm_render_texrect(pe->dropdown->x, pe->dropdown->y, pe->dropdown->w, pe->dropdown->h, 32, 32, 0x20, 0x20, 0x20, 0xF0, OMM_TEXTURE_MISC_WHITE, false);
            for (s32 i = 0; i != pe->dropdown->count; ++i) {
                s32 y = OMM_PE_DROPDOWN_TEXT_Y(i);
                if (i == pe->dropdown->selected) {
                    omm_render_texrect(OMM_PE_DROPDOWN_HOVER_X, OMM_PE_DROPDOWN_HOVER_Y(y), OMM_PE_DROPDOWN_HOVER_W, OMM_PE_DROPDOWN_HOVER_H, 32, 32, 0x80, 0x80, 0x80, 0xC0, OMM_TEXTURE_MISC_WHITE, false);
                }
                u8 *textChoice = omm_text_convert(pe->dropdown->choices[i], false);
                omm_palette_editor_render_string(textChoice, OMM_PE_STR_MAX_LENGTH_RIGHT, OMM_PE_TEXT_X2, y, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
            }
            ...
            omm_palette_editor_render_string(textInput, textInputLength, textInputX, textInputY, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
            for (s32 i = 0; i != 6; ++i) {
                ...
                if (i < pe->input->index) {
                    ...
                    omm_palette_editor_render_string(textDigit, 1, inputBoxX + OMM_PE_INPUT_BOX_CHAR_MARGIN, textInputY, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0x00, 0x00, 0x00, 0xFF, false);
                }
            }
            ...
            for (s32 palette = 0; palette != pe->dropdown->count; ++palette) {
                ...
                omm_palette_editor_render_string(textPaletteName, OMM_PE_STR_MAX_LENGTH_PALETTES, textPaletteNameX, textPaletteNameY, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xFF, 0xFF, 0xFF, 0xFF, true);
            }
            const char *paletteStr = omm_mario_colors_choices(pe->peach)[omm_mario_colors_count() - 4 + pe->palette];
            ...
            omm_palette_editor_render_string(textCopyPalette1, textCopyPaletteLength1, textCopyPalette1X, textCopyPaletteY, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
            omm_palette_editor_render_string(textCopyPalette2, textCopyPaletteLength2, textCopyPalette2X, textCopyPaletteY, OMM_PE_CHAR_WIDTH, OMM_PE_CHAR_HEIGHT, 0xD0, 0xD0, 0xD0, 0xFF, true);
        }
...
static bool omm_palette_editor_update_color_input(ev *event, s32 group, s32 boxx, s32 boxy, s32 boxw, s32 boxh) {
    if (event != OMM_PE_EVENT_NONE && pe->event == event && (pe->group != 0 || pe->subgroup == group)) {
        pe->hover->x = boxx;
        pe->hover->y = boxy;
        pe->hover->w = boxw;
        pe->hover->h = boxh;
    ...
    if (pe->event == OMM_PE_EVENT_COPY_PALETTE) {
        ...
        if (OMM_PE_INPUT_A_PRESSED || OMM_PE_INPUT_B_PRESSED) {
            ...
            if (!OMM_PE_INPUT_B_PRESSED && pe->dropdown->selected >= 0 && pe->dropdown->selected < pe->dropdown->count) {
                for (s32 i = 0; i != omm_mario_colors_lights_count(pe->peach); ++i) {
                    mem_cpy(
...
static void omm_palette_editor_update_group() {
    ...
    for (s32 i = 0; i != numGroups; ++i) {
        OMM_PALETTE_EDITOR_GROUP_CHOICES[1 + i] = omm_mario_colors_get_light_group_name(pe->peach, i);
    }
...
static void omm_palette_editor_update_color_inputs() {
    if (pe->group == 0) {
        ...
        for (s32 group = 0; group != numGroups; ++group) {
            ...
            // Select group
            if (omm_palette_editor_update_color_input(OMM_PE_EVENT_NONE, group, OMM_PE_HOVER_BOX_X_0, OMM_PE_HOVER_BOX_Y(y), OMM_PE_HOVER_BOX_W_0, OMM_PE_HOVER_BOX_H)) {
                omm_palette_editor_update_current();
                return;
            }

            // Edit group color 1
            if (omm_palette_editor_update_color_input(OMM_PE_EVENT_INPUT_COLOR_1, group, OMM_PE_HOVER_BOX_X_1, OMM_PE_HOVER_BOX_Y(y), OMM_PE_HOVER_BOX_W_1, OMM_PE_HOVER_BOX_H)) {
                omm_palette_editor_update_current();
                return;
            }

            // Edit group color 2
            if (omm_palette_editor_update_color_input(OMM_PE_EVENT_INPUT_COLOR_2, group, OMM_PE_HOVER_BOX_X_2, OMM_PE_HOVER_BOX_Y(y), OMM_PE_HOVER_BOX_W_2, OMM_PE_HOVER_BOX_H)) {
                omm_palette_editor_update_current();
                return;
            }
        }
        ...
        if (omm_palette_editor_update_color_input(OMM_PE_EVENT_INPUT_COLOR_1, pe->group, OMM_PE_HOVER_BOX_X_1, OMM_PE_HOVER_BOX_Y(OMM_PE_TEXT_COLOR_1_Y), OMM_PE_HOVER_BOX_W_1_2, OMM_PE_HOVER_BOX_H)) {
            omm_palette_editor_update_current();
        }

        // Edit current group color 2
        if (omm_palette_editor_update_color_input(OMM_PE_EVENT_INPUT_COLOR_2, pe->group, OMM_PE_HOVER_BOX_X_1, OMM_PE_HOVER_BOX_Y(OMM_PE_TEXT_COLOR_2_Y), OMM_PE_HOVER_BOX_W_1_2, OMM_PE_HOVER_BOX_H)) {
            omm_palette_editor_update_current();
        }
    }
}
